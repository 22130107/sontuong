...
export default function AdminProjectsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) setProjects(result.data.items);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleDelete = async (id: number) => {
    ...
    const result = await res.json();
    if (result.success) { showToast("Đã xóa công trình"); load(); }
      ...
      <div className="flex min-h-screen">
        ...
        <main className="flex-1 flex flex-col overflow-hidden">
          <header className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between flex-shrink-0">
            <div>
              <h1 className="text-xl font-bold text-gray-800">Quản lý công trình</h1>
              <p className="text-sm text-gray-500 mt-0.5">{projects.length} công trình</p>
            </div>
            <Link href="/admin/cong-trinh/them"
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
              {Object.entries(CATEGORY_LABELS).map(([key, label]) => (
                <div key={key} className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm">
                  <p className="text-2xl font-black" style={{ color: "#1a3a8f" }}>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-200" style={{ backgroundColor: "#f8faff" }}>
                      ...
                      <th className="text-left px-4 py-3 font-semibold text-gray-600">Công trình</th>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {filtered.length === 0 ? (
                      <tr><td colSpan={4} className="text-center py-12 text-gray-400">
                        <div className="text-4xl mb-2">🏗️</div><p>Không tìm thấy công trình nào</p>
                      </td></tr>
                      ...
                      <tr key={project.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-3">
                            <div className="relative w-14 h-10 flex-shrink-0 rounded-lg overflow-hidden border border-gray-100 bg-gray-50">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center justify-end gap-2">
                            <Link href={`/cong-trinh/${project.slug}`} target="_blank"
                            ...
                            <Link href={`/admin/cong-trinh/${project.id}`}
      ...
      {deleteId !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm">
            ...
            <p className="text-sm text-gray-500 text-center mb-6">Bạn có chắc muốn xóa công trình này?</p>
